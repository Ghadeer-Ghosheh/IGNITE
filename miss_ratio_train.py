# -*- coding: utf-8 -*-
"""
Created on Mon Feb 13 06:59:04 2023
...
import argparse
from downstream_eval import get_sets_feature_missingess
...
def main (args):
    ...
    with open(os.path.join(data_path, 'mask_miss_0.1_'+str(seed)+'.pkl'), 'rb') as f:

        miss = pickle.load(f)
    with open(os.path.join(data_path, 'new_miss_0.1_back_forward_'+str(seed)+'.pkl'), 'rb') as f:

    #with open(os.path.join(data_path, 'backforward.pkl'), 'rb') as f:
        IMM_input = pickle.load(f)
    
    with open(os.path.join(data_path, 'new_miss_oo_0.1_'+str(seed)+'.pkl'), 'rb') as f:
        origional = pickle.load(f)
     
    with open(os.path.join(data_path, 'IMM_0.1_'+str(seed)+'.pkl'), 'rb') as f:
    ...
    #IMM_vae_x = IMM_input[:sample_size]
...
if __name__ == '__main__':  
   exp_name = "final_loss_contrastive_added_to_top_miss_01_pop_30_epoch"
